"""
Fixes the "Atlas Invoice Tracking Classic" Sales Invoice print format:

  1. Line items and totals were re-derived from the LIVE Item Price /
     items_per_case at PDF-render time instead of the invoice's own saved
     rate/amount/net_total/grand_total. Since pricing syncs every minute
     (see setup_stock_price_scheduled_sync.py), an invoice printed after a
     price change silently showed today's price instead of the price that
     was actually on the Sales Order when it was submitted. Now uses
     it.rate / it.amount / doc.net_total / doc.grand_total, which ERPNext's
     native Sales Order -> Sales Invoice mapping (make_sales_invoice) copies
     from the Sales Order at submission time and never re-prices.
  2. A malformed nested `<style><style>...</style>` tag (left behind by an
     earlier ad-hoc fix) is now a single well-formed <style> block.
  3. Terms & Conditions (.btm-terms) no longer uses `position: fixed`.
     wkhtmltopdf does not reliably support fixed-position body content
     across multi-page documents -- confirmed in production, where it
     rendered on top of (overlapping) real item rows instead of being
     reserved its own space on the page. T&C now flows normally right
     after the totals block, so it prints once at the true end of the
     invoice with zero risk of overlapping table content. The separate
     page-number footer (.inv-footer) is dropped -- the page number is
     already shown in the top meta-box.

Usage:
  ERP_ADMIN_USER=... ERP_ADMIN_PWD=... python _fix_invoice_pdf_pricing_and_layout.py
"""
